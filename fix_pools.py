...
old_pool = "const textPool = studyTopics.flatMap(t => t.content).filter(c => c.split(' ').length > 20 && c.toUpperCase() !== c).map(c => c.replace(/\\s+/g, ' ').trim());"
...
spell_pool = "const textPool = studyTopics.flatMap(t => t.content).filter(c => c.split(' ').length > 15 && c.toUpperCase() !== c).map(c => c.replace(/\\s+/g, ' ').trim());"
spelling_content = spelling_content.replace(spell_pool, new_pool)
